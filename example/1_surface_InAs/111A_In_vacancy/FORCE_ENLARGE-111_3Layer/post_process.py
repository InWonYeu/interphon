...
class PostProcess(PreProcess):
    ...
    def set_force_constant(self, force_files: FilePath, code_name: str = 'vasp') -> None:
        ...
        if _ind_pbc.shape[0] == 2:
            W_select, w_select, same_index_select, point_group_ind, require, not_require = symmetry_2d(self.super_cell)
            # require = [_ind for _ind, _ in enumerate(self.unit_cell.atom_true) if _ind not in not_require]
            print(point_group_ind)
            print(not_require)
            print(require)
            ...
            if code_name == 'vasp':
                ...
                for _point_group_ind, _not_require in zip(point_group_ind, not_require):
                    # print(W_select[_point_group_ind])
